...
def gdal_write(ary, geoTransform, fileformat="GTiff", filename='jupyter_rocks.tif',
               data_format=gdal.GDT_Float64, nodata=None,
               srs_wkt='GEOGCS["WGS 84",DATUM["WGS_1984",SPHEROID["WGS 84",6378137,298.257223563,'
...
def calculate_hand(dem_array, dem_affine: rasterio.Affine, dem_crs: rasterio.crs.CRS, basin_mask,
    ...
    with NamedTemporaryFile() as temp_file:
        ...
        #gdal_write(dem_array, dem_affine.to_gdal(), filename=temp_file.name,
        #           srs_wkt=dem_crs.to_wkt(), nodata=nodata_fill_value)

        dem_nodata_value = 0
        dem_nodata_mask = dem_array == dem_nodata_value
    ...
    if np.isnan(hand).any():
        ...
        hand[np.logical_or(basin_mask, dem_nodata_mask)] = nodata_fill_value
...
def calculate_hand_for_basins(out_raster:  Union[str, Path], geometries: GeometryCollection,
    ...
    with rasterio.open(dem_file) as src:
        ...
        out_raster_cog = f'{out_raster.split(".tif")[0]}_cog.tif'

        write_cog(
              out_raster_cog, hand, transform=basin_affine_tf.to_gdal(), epsg_code=src.crs.to_epsg(), nodata_value=np.nan,
        )

        out_raster_gdal = f'{out_raster.split(".tif")[0]}_gdal.tif'

        gdal_write(hand, basin_affine_tf.to_gdal(), filename=out_raster_gdal,
                   srs_wkt=src.crs.to_wkt(), nodata=np.nan)
