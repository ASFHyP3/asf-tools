...
dem_file = "/media/jzhu4/data/hand/review_from_hydrosar/test1/Copernicus_DSM_COG_10_N24_00_E088_00_DEM.tif"
...
for k, p in enumerate(tqdm(polygons)):
    ...
    with warnings.catch_warnings():
        warnings.filterwarnings("ignore", category=FutureWarning)
        h, acc_tmp = calculate_hand(np.squeeze(dem.read(window=win)), tf, pyproj.Proj(init=dem.crs.to_string()), mask=not_mask, verbose=verbose, acc_thresh=accumulation_threshold)
    ...
    acc[win.row_off:win.row_off+win.height, win.col_off:win.col_off+win.width] = acc_tmp
...
# write acc to a file.
hand_file=os.path.splitext(dem_file)[0]+f"_acc_{version.replace('.','_')}.tif"
gdal_write(acc, dem_gT, filename=hand_file, srs_proj4=dem_proj4, nodata=np.nan, data_format=gdal.GDT_Float32)
