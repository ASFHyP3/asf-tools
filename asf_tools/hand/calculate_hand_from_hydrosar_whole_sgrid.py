...
def calculate_hand_sgrid(dem_file: Union[str, Path], mask=None, acc_thresh: Optional[int] = 100):
    ...
    ### test nodata and mask, original nodata=None, set mask, apply mask
    #grid.nodata must be a numeric type, while in pgrid, grid.nodata is None

    #grid.nodata = np.nan, if not set, grid.nodata=0

    #grid.nodata = np.finfo(float).eps
...
'''
# original hydrosar loop-over method
...
# test pgrid and no-loop method

mask, tf, win = rasterio.mask.raster_geometry_mask(dem, polygons, crop=True,
                                                 pad=True, pad_width=pad_width)
not_mask = np.bitwise_not(mask)
hand = calculate_hand(np.squeeze(dem.read(1)), tf,
                  pyproj.Proj(init=dem.crs.to_string()),
                  mask=not_mask, verbose=False,
                  acc_thresh=accumulation_threshold)

'''

# test sgrid and no-loop method
mask, tf, win = rasterio.mask.raster_geometry_mask(dem, polygons, crop=True,
                                                   pad=True, pad_width=pad_width)
not_mask = np.bitwise_not(mask)

hand = calculate_hand_sgrid(dem_file, mask=not_mask, acc_thresh=accumulation_threshold)
