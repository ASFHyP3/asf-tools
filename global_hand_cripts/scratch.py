...
def main_fun(dem_tile, acc_thresh, s3_bucket, s3_prefix, out_dir, verbose=False):
    ...
    log.info(f'Making HAND tile from {dem_tile}')

    hand_raster = make_asf_hand(dem_tile, acc_thresh=acc_thresh)
    os.system(f'mv {hand_raster} {out_dir}')

    log.info(f'HAND tile COG created successfully: {hand_raster}')

    if s3_bucket:
        key = f'{s3_prefix}/{hand_raster}' if s3_prefix else str(hand_raster)
        log.info(f'Uploading HAND tile to s3://{s3_bucket}/{key}')
        #S3.upload_file(hand_raster, s3_bucket, key)
        S3.upload_file(f"{out_dir}/{hand_raster}", s3_bucket, key)
