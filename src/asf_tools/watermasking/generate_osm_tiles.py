...
def process_pbf(planet_file: str, output_file: str):
    ...
    natural_water_command = (
        f"osmium tags-filter -o {natural_file} {planet_file} wr/natural=water".split(
            " "
        )
    )
    waterways_command = (
        f'osmium tags-filter -o {waterways_file} {planet_file} waterway="*"'.split(" ")
    )
...
def process_ocean_tiles(
    ocean_polygons_path, lat, lon, tile_width_deg, tile_height_deg, output_dir
):
...
def extract_water(
    water_file, lat, lon, tile_width_deg, tile_height_deg, interior_tile_dir
):
    ...
    export_command = (
        f"osmium export --geometry-types=polygon {tile_pbf} -o {tile_geojson}".split(
            " "
        )
    )
...
def merge_interior_and_ocean(
    internal_tile_dir, ocean_tile_dir, finished_tile_dir, translate_to_cog: bool = False
):
    ...
    for filename in os.listdir(internal_tile_dir):
        if filename.endswith(".tif"):
            ...
            if translate_to_cog:
                ...
                translate_string = (
                    "gdal_translate -ot Byte -of COG -co NUM_THREADS=all_cpus"
                )
...
def main():
    ...
    parser.add_argument(
        "--planet-file-path", help="The path to the global planet.pbf file."
    )
    parser.add_argument(
        "--ocean-polygons-path", help="The path to the global OSM ocean polygons."
    )
    parser.add_argument(
        "--lat-begin",
    ...
    try:
        ...
        raise ImportError(
            "osmium-tool must be installed to run this program: https://osmcode.org/osmium-tool/."
        )
    ...
    for lat in lat_range:
        for lon in lon_range:
            ...
            total_time = end_time - start_time
            print(
                f"Finished initial creation of {tile_name} in {total_time}(s). {index} of {num_tiles}"
            )
            index += 1

    print("Merging processed tiles...")
    merge_interior_and_ocean(
        INTERIOR_TILE_DIR, OCEAN_TILE_DIR, FINISHED_TILE_DIR, translate_to_cog=True
    )
