...
def get_waterbody(input_info: dict, threshold: Optional[float] = None) -> np.array:
    ...
    with tempfile.NamedTemporaryFile() as water_extent_file:
        gdal.Warp(
            water_extent_file.name,
            str(water_extent_vrt),
            dstSRS=f"EPSG:{epsg}",
...
def iterative(
    ...
    def get_confusion_matrix(w):
        ...
        tp = np.nansum(
            np.logical_and(iterative_flood_extent == 1, extent == 1)
        )  # true positive
        tn = np.nansum(
            np.logical_and(iterative_flood_extent == 0, extent == 0)
        )  # true negative
        fp = np.nansum(
            np.logical_and(iterative_flood_extent == 1, extent == 0)
        )  # False positive
        fn = np.nansum(
            np.logical_and(iterative_flood_extent == 0, extent == 1)
        )  # False negative
        return tp, tn, fp, fn
    ...
    def _goal_ts(w):
        ...
        return 1 - tp / (
            tp + fp + fn
        )  # threat score -- we will minimize goal func, hence `1 - threat_score`.
    ...
    with warnings.catch_warnings():
        ...
        opt_res = optimize.basinhopping(
            MINIMIZATION_FUNCTION[minimization_metric],
            np.mean(water_levels),
            niter=10000,
            niter_success=100,
            accept_test=bounds,
            stepsize=3,
    ...
    if (
        opt_res.message[0] == "success condition satisfied"
        or opt_res.message[0]
...
def estimate_flood_depth(
    ...
    with warnings.catch_warnings():
        ...
        if estimator.lower() == "nmad":
            ...
            hand_std = stats.median_abs_deviation(
                hand[flood_labels == label], scale="normal", nan_policy="omit"
            )
...
def make_flood_map(
    ...
    *Numpy: Calculates statistics on a linear scale. Least robust to outliers and non-Gaussian
    ...
    for ll in tqdm(range(1, num_labels)):  # Skip first, largest label.
        ...
        flood_depth_window[flood_window == ll] = (
            water_height - hand_window[flood_window == ll]
        )
...
def _get_cli(interface: Literal["hyp3", "main"]) -> argparse.ArgumentParser:
    ...
    if interface == "hyp3":
        ...
        parser.add_argument(
            "--wm-raster", help="Water map GeoTIFF raster, with suffix `_WM.tif`."
        )
        available_estimators.append(None)
        estimator_help += " If `None`, flood depth will not be calculated."
    elif interface == "main":
        parser.add_argument(
            "out_raster", help="File to which flood depth map will be saved."
        )
        parser.add_argument(
            "vv_raster",
        ...
        parser.add_argument(
            "water_extent_map", help="HyP3-generated water extent raster file."
        )
        parser.add_argument(
            "hand_raster",
...
def hyp3():
    ...
    if args.wm_raster:
        ...
        water_map_raster = get_path_to_s3_file(
            args.bucket, args.bucket_prefix, "_WM.tif"
        )
    ...
    make_flood_map(
        out_raster=flood_map_raster,
        vv_raster=vv_raster,
        water_raster=water_map_raster,
        hand_raster=hand_raster,
        estimator=args.estimator,
        water_level_sigma=args.water_level_sigma,
        known_water_threshold=args.known_water_threshold,
        iterative_bounds=(args.iterative_min, args.iterative_max),
        iterative_min_size=args.iterative_min_size,
...
def main():
    ...
    make_flood_map(
        out_raster=args.out_raster,
        vv_raster=args.vv_raster,
        water_raster=args.water_extent_map,
        hand_raster=args.hand_raster,
        estimator=args.estimator,
        water_level_sigma=args.water_level_sigma,
        known_water_threshold=args.known_water_threshold,
        iterative_bounds=tuple(args.iterative_bounds),
        iterative_min_size=args.iterative_min_size,
        minimization_metric=args.minimization_metric,
    )
