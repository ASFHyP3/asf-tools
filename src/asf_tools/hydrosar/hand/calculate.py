...
def calculate_hand_for_basins(
    ...
    with rasterio.open(dem_file) as src:
        ...
        hand = calculate_hand(
            basin_array, basin_affine_tf, src.crs, basin_mask, acc_thresh=acc_thresh
        )
...
def make_copernicus_hand(
    ...
    with NamedTemporaryFile(suffix=".vrt", delete=False) as dem_vrt:
        ...
        calculate_hand_for_basins(
            out_raster, geometries, dem_vrt.name, acc_thresh=acc_thresh
        )
...
def main():
    ...
    make_copernicus_hand(
        args.out_raster, args.vector_file, acc_thresh=args.acc_threshold
    )
