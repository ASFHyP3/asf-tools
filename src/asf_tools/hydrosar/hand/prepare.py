...
HAND_GEOJSON = (
    "/vsicurl/https://glo-30-hand.s3.amazonaws.com/v1/2021/glo-30-hand.geojson"
)
...
def prepare_hand_vrt(
    vrt: Union[str, Path], geometry: Union[ogr.Geometry, BaseGeometry]
):
    ...
    with GDALConfigManager(GDAL_DISABLE_READDIR_ON_OPEN="EMPTY_DIR"):
        ...
        if not vector.get_property_values_for_intersecting_features(
            ...
            raise ValueError(
                f"Copernicus GLO-30 HAND does not intersect this geometry: {geometry}"
            )

        hand_file_paths = vector.intersecting_feature_properties(
