...
def test_select_hand_tiles(hand_candidates):
    ...
    hand_tiles = np.ma.masked_invalid(
        tile_array(hand_array, tile_shape=(100, 100), pad_value=np.nan)
    )

    selected_tiles = water_map.select_hand_tiles(hand_tiles, 15.0, 0.8)
    assert np.all(selected_tiles == hand_candidates)

    with pytest.raises(ValueError):
        _ = water_map.select_hand_tiles(
            np.zeros(shape=(10, 10, 10), dtype=float), 15.0, 0.8
        )
...
def test_select_backscatter_tiles(hand_candidates):
    ...
    backscatter_tiles = np.ma.masked_less_equal(
        tile_array(backscatter_array, tile_shape=(100, 100), pad_value=0.0), 0.0
    )

    tile_indexes = water_map.select_backscatter_tiles(
        backscatter_tiles, hand_candidates
    )
