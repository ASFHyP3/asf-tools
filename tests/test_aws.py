...
def test_get_tag_set():
    assert aws.get_tag_set() == {"TagSet": [{"Key": "file_type", "Value": "product"}]}
...
def test_upload_file_to_s3(tmp_path, s3_stubber):
    ...
    s3_stubber.add_response(
        method="put_object", expected_params=expected_params, service_response={}
    )
    s3_stubber.add_response(
        method="put_object_tagging", expected_params=tag_params, service_response={}
    )
...
def test_get_path_to_s3_file(s3_stubber):
    ...
    assert (
        aws.get_path_to_s3_file("myBucket", "myPrefix", file_type=".nc")
        == "/vsis3/myBucket/myPrefix/foo.nc"
    )
    ...
    assert (
        aws.get_path_to_s3_file("myBucket", "myPrefix", file_type=".txt")
        == "/vsis3/myBucket/myPrefix/foo.txt"
    )
